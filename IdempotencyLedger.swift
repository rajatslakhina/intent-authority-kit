...
public struct IdempotencyLedger: Sendable {
    ...
    /// Records a *new* key. Returns `false` if it could not be stored, in which
    /// case nothing changed and the caller must refuse.
    ///
    /// **Refuses to overwrite a record that is still inside the retry horizon.**
    /// Such an overwrite would reset `recordedAt` — silently extending the
    /// horizon — and would clobber a settled outcome back to `.inFlight`, so a
    /// later genuine retry would be told the wrong thing. Callers that mean to
    /// update an outcome use `settle`, which preserves the original timestamp.
    ///
    /// A record *past* the horizon is replaced, which is the intended behaviour:
    /// past the horizon a repeat is a new request, not a retry.
    @discardableResult
    public mutating func record(
        ...
        guard replay(of: key, now: now) == nil else { return false }
    ...
    /// The recorded outcome for `key`, ignoring the horizon. Used by `rollback`,
    /// which must refuse to undo a commit that already settled regardless of age.
    public func recordedOutcome(of key: IdempotencyKey) -> CommitOutcome? {
        records[key]?.outcome
    }
}
