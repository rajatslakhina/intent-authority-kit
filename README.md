...
In iOS 27, Siri decides at runtime which registered intent satisfies a request and chains several together. That makes your intent surface a public API invoked by a caller you do not control, do not authenticate, and cannot patch. NowSecure's August 2026 teardown names the failure modes: *parameter poisoning* (the recipient field of a transfer is rewritten by injected content), *action poisoning* (the planner is steered to fire `openURL` instead of `summarize`), and the one that makes both worse — the **baton pass**, where one agent hands off to another inside a session and the full transcript, injection included, is inherited downstream. Apple's own WWDC26 session 347 concedes indirect prompt injection is unsolved and pushes threat modelling onto the developer.
...
**`DefaultAuthorizationPolicy`** — five independent pure checks (content, selection, blast radius, budget, tier) joined **worst-wins**. `ConfirmationRequirement` is `Comparable` and combined with `max`, so evaluation order provably cannot change the answer — and that property is itself a shipped, runnable check.

**`ConfirmationReceipt`** — a digest over the exact resolved parameters, *not* a boolean. App Intents re-resolves parameters and planners retry, so between the moment the user approved and the moment the commit runs, the values can legitimately change. A flag cannot tell; a digest can. Blast radius is inside the digest, so a receipt granted for "archive this 1 message" does not satisfy a re-resolution to 400.
...
**`IdempotencyLedger`** — bounded, with eviction permitted only past the retry horizon. Dropping a record re-admits a retry as a fresh commit and double-commits, so the ledger **refuses** rather than evicting under pressure.
...
**The commit budget is taken before the confirmation `await`.** Actor isolation gives mutual exclusion *between* suspension points and nothing across them, and `await presenter.confirm(...)` sits in the middle of a check-then-act sequence. The wrong order is shipped as `CommitAdmissionOrder.reserveAfterConfirmation` and proved wrong by a barrier-driven test: with a budget of 1 and four concurrent invocations, the naive order raises **four** prompts the session can never honour and the correct order raises **one**. That is a confirmation-fatigue vector an attacker can drive on purpose.
...
- `swift test` — **72 tests, 0 failures.** The concurrency tests were re-run five times to check for scheduling flakiness; stable.
