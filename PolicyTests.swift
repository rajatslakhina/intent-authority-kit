...
final class PolicyTests: XCTestCase {
    ...
    func testMostSevereCheckWins() {
        let both = requirement(
            provenance: .contentDerived(SourceID("m")), floor: .contentExposed, radius: 0
        )
        // Named literally rather than recomputed with `combined`, which would
        // just be the implementation asserting about itself. Of the two refusals
        // in play, `emptyEffect` wins the tie-break because refusals of equal
        // severity are ordered by reason so that `max` is total and deterministic.
        XCTAssertEqual(both, .refuse(.emptyEffect))
    }

    /// The tier gate is *not* part of the order-independent join, and pretending
    /// otherwise is the mistake the policy's doc comment now warns about: an
    /// empty read is a fact about the world, an empty commit is a planner error.
    func testTierGateShortCircuitsAheadOfTheJoin() {
        XCTAssertEqual(requirement(tier: .read, radius: 0), .none)
        XCTAssertEqual(requirement(tier: .propose, radius: 0), .none)
        XCTAssertEqual(requirement(tier: .commit, radius: 0), .refuse(.emptyEffect))
    }
