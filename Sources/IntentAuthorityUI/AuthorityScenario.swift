...
public extension AuthorityScenario {
    ...
    /// The five cases that separate this design from the obvious one.
    static let catalog: [AuthorityScenario] = [
        ...
        AuthorityScenario(
            ...
            note: "Same intent, same effect class, same perform() body — at 400 targets. Blast radius belongs to the resolved set, not the verb.",
            floor: .plannerOnly,
            invocation: makeInvocation(
                intent: "mail.archive", tier: .commit,
