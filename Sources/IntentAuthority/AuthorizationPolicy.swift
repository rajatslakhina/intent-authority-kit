...
public struct AuthorityLimits: Sendable, Hashable {
    ...
    /// Maximum distinct ingested content sources a session will remember.
    public let ingestedSourceCapacity: Int
    ...
    public init(
        ...
        self.ingestedSourceCapacity = max(1, Saturating.nonNegative(ingestedSourceCapacity))
    }
...
/// The default policy.
///
/// One **tier gate**, then **four independent checks joined worst-wins**.
///
/// The distinction matters and is not cosmetic. The four joined checks (content,
/// selection, blast radius, budget) are pure functions combined with `max`, so
/// their evaluation order provably cannot change the answer — that is what
/// `AuthorityAudit.requirementJoinIsOrderIndependent` proves. The tier gate is a
/// deliberate short circuit *in front of* the join, and it is order-dependent by
/// construction: a `.read` with zero resolved entities returns `.none`, not
/// `.refuse(.emptyEffect)`, because an empty read is a fact about the world and
/// an empty commit is a planner error. Do not mistake the proof about the join
/// for a proof about the gate.
